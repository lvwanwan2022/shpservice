...
--我希望添加连接或者添加完成后任何时候点击测试连接，网页能否不用请求后端，直接从用户的网页端请求Geoserver的服务器连接，根据Geoserver的返回情况判断连接是否正常。这样即使本系统的服务器S与Geoserver的服务器A不能联通，只要前端网页所在的电脑B和Geoserver所在的服务器A联通，前端B可以直接使用A服务器上的服务
-- =============================================
