...
// 缓存相关导入
import { createTileCache, createOpenLayersAdapter } from '@/services/tileCache/index.js'
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const initializeTileCache = async () => {
      try {
        ...
        // 创建OpenLayers适配器
        olCacheAdapter.value = createOpenLayersAdapter({
          cacheService: tileCache.value,
          enableCache: true,
        ...
        setInterval(() => {
          if (olCacheAdapter.value) {
            const stats = olCacheAdapter.value.getRetryStats()
            if (stats.activeRetries > 0 || stats.pendingRequests > 0) {
              console.log('🔄 缓存适配器状态:', {
            ...
            // 如果重试太多，清理一些旧的状态
            if (stats.activeRetries > 100) {
              console.warn('⚠️ 重试太多，清理状态')
              olCacheAdapter.value.clearRetryCounters()
            }
          }
        }, 30000) // 每30秒检查一次
        ...
        // 即使缓存初始化失败，也不应该阻止地图初始化
        // 设置为null，让地图使用标准模式
        tileCache.value = null
        olCacheAdapter.value = null
        return false
      }
    }
    ...
    const initMap = async () => {
      ...
      // 4. 确保OpenLayers在window对象上可用（用于缓存适配器）
      if (typeof window !== 'undefined') {
        if (!window.ol) {
          window.ol = {
            source: { XYZ, TileWMS, VectorTile },
 ...
 // 设置GCJ02的有效范围（基于中国区域）
        ...
        // 高德地图 - 使用GCJ02坐标系修正偏移（支持缓存）
        let gaodeSource;
        if (olCacheAdapter.value) {
          const cachedSource = olCacheAdapter.value.createCachedXYZSource('gaode', {
          ...
          gaodeSource = cachedSource || new XYZ({
            url: 'https://webrd01.is.autonavi.com/appmaptile?lang=zh_cn&size=1&scale=1&style=8&x={x}&y={y}&z={z}',
            crossOrigin: 'anonymous',
            projection: gcj02Mecator,
            maxZoom: 18,
        ...
        // 高德卫星地图 - 使用GCJ02坐标系修正偏移（支持缓存）
        let gaodeSatelliteSource;
        if (olCacheAdapter.value) {
          const cachedSource = olCacheAdapter.value.createCachedXYZSource('gaode-satellite', {
          ...
          gaodeSatelliteSource = cachedSource || new XYZ({
            url: 'https://webst01.is.autonavi.com/appmaptile?style=6&x={x}&y={y}&z={z}',
            crossOrigin: 'anonymous',
            projection: gcj02Mecator,
            maxZoom: 18,
        ...
        let osmSource;
        if (olCacheAdapter.value) {
          const cachedSource = olCacheAdapter.value.createCachedXYZSource('osm', {
          ...
          osmSource = cachedSource || new XYZ({
            url: 'https://tile.openstreetmap.org/{z}/{x}/{y}.png',
            crossOrigin: 'anonymous',
            maxZoom: 19,
            minZoom: 1,
        ...
        // Esri 世界影像（卫星图，支持缓存）
        let esriSatelliteSource;
        if (olCacheAdapter.value) {
          const cachedSource = olCacheAdapter.value.createCachedXYZSource('esri-satellite', {
          ...
          esriSatelliteSource = cachedSource || new XYZ({
            url: 'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
            crossOrigin: 'anonymous',
            maxZoom: 21,
            minZoom: 1,
    ...
    const addMartinLayer = async (layer) => {
      ...
      try {
        ...
        if (isRasterMbtiles) {
          // 创建栅格XYZ图层 - 用于栅格mbtiles（支持缓存）
          const layerCacheKey = `martin-raster-${layer.id}-${layer.layer_name}`;
          
          let rasterSource;
          if (olCacheAdapter.value) {
            const cachedSource = olCacheAdapter.value.createCachedXYZSource(layerCacheKey, {
              url: mvtUrl,
              maxZoom: 22,
              minZoom: 0,
              wrapX: false,
            ...
            rasterSource = cachedSource || new XYZ({
              url: mvtUrl,
              maxZoom: 22,
              minZoom: 0,
              wrapX: false,
          ...
          if (olCacheAdapter.value) {
            ...
            olLayer = new VectorTileLayer({
              ...
              source: new VectorTile({
                ...
                cacheSize: 128,
                // 使用缓存的瓦片加载函数
                tileLoadFunction: olCacheAdapter.value.createTileLoadFunction(layerCacheKey, mvtUrl)
              }),
